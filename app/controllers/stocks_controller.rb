class StocksController < ApplicationController
  ...
  if current_user
    @total_shares = 0
    @total_shares_ded = 0
    @stock.purchases.where(:user_id => current_user.id).each do |purchase|
     @total_shares = @total_shares + purchase.shares 
  end

  @stock.sales.where(:user_id => current_user).each do |sale|
  ...
  private
    ...
    def stock_params
      params.require(:stock).permit(:name, :total_shares, :shares_left, :value)
    end
end
